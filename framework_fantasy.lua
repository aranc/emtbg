-- All the code in this file is adapted from https://github.com/karthikncode/text-world-player
-- Please see license (MIT) at: https://github.com/karthikncode/text-world-player/blob/main/LICENSE.txt
...
local OBELISK_PASSED = false
local ANTECHAMBER_PASSED = false
...
function parse_game_output(text)
  ...
  local terminal = false
	for i=1, #text do
      ...
      if BRIDGE_PASSED and string.match(text[i], "REWARD_bridge") then
        valid_reward = false
      end
      if OBELISK_PASSED and string.match(text[i], "REWARD_obelisk") then
        valid_reward = false
      end
      if ANTECHAMBER_PASSED and string.match(text[i], "REWARD_antechamber") then
        valid_reward = false
      end

      if string.match(text[i], "REWARD_success") then
        terminal = true
      end
      if string.match(text[i], "REWARD_fail") then
        terminal = true
      end
      ...
      if valid_reward then
			...
			if string.match(text[i], "REWARD_obelisk") then
				OBELISK_PASSED = true
			end
			if string.match(text[i], "REWARD_antechamber") then
				ANTECHAMBER_PASSED = true
			end
	...
	return text_to_agent, reward, exits, objects_available, terminal
end
...
function newGame(gameLogger)
	data_out("@teleport tut#03") --teleport to somewhere that is not the cliff, to avoid getting "root is already here" error message
  data_in()
	data_out("@teleport tut#02") --teleport to Cliff by the coast	
	data_out("@resetpuzzle")
	data_out("look") 
	sleep(0.5)
	STEP_COUNT = 0
	BRIDGE_PASSED = false
	OBELISK_PASSED = false
	ANTECHAMBER_PASSED = false
...
function build_command(action, object, logger)
  ...
  if action == 'go' and object == 'obelisk' then
    return "look obelisk"
  end
...
function findObjectIndices(list)
	...
	for i,s in pairs(list) do
		s = s:trim()
    s = s:gsub("%d+m", "") --Aran: extra trim
		s = s:trim() --Aran: extra trim
...
function getState(logger, prev_command)
  ...
  if false then
	...
	local text, reward, exits, objects_available, _terminal = parse_game_output(inData)		
  if _terminal then
    terminal = true
  end
  ...
  if reward == JUNK_CMD_REWARD or (prev_command and prev_command ~='go') then
		...
		local text2, tmp_reward, exits2, objects_available2, _terminal = parse_game_output(inData2) -- the room description after 'looking'
    if _terminal then
      terminal = true
    end
		if text ~= text2 then		
